...
export const metadata: Metadata = {
  title: "Εκτύπωση Σχεδίων | Φωτοαντιγραφικό Κέντρο Ηλιούπολης",
  description:
    "Εκτύπωση, αναπαραγωγή και ψηφιοποίηση τοπογραφικών & αρχιτεκτονικών σχεδίων σε μεγάλο format. A0, A1, A2, A3.",
};

const services = [
  { key: "digitalPrinting", icon: Printer },
  { key: "photocopying", icon: Copy },
  { key: "scanning", icon: ScanLine },
  { key: "paper", icon: ScrollText },
  { key: "folding", icon: FileType },
  { key: "extras", icon: FolderOpen },
] as const;

const formats = ["PDF", "TIFF", "JPG", "PNG"];
const sizes = ["A3", "A2", "A1", "A0", "90cm+"];

const BlueprintsPage = async ({ params }: BasePageProps) => {
  const { locale } = await params;
  setRequestLocale(locale);
  const t = await getTranslations("BlueprintsPage");
