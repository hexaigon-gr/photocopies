...
export const metadata: Metadata = {
  title: "Φωτογραφίες Ταυτότητας | Φωτοαντιγραφικό Κέντρο Ηλιούπολης",
  description:
    "Φωτογραφίες για ταυτότητα, διαβατήριο, δίπλωμα οδήγησης και κάθε επίσημο έγγραφο — άμεσα και σε σωστές προδιαγραφές.",
};

const services = [
  { key: "idCard", icon: CreditCard },
  { key: "passport", icon: BookOpen },
  { key: "drivingLicense", icon: Car },
] as const;

const IdPhotosPage = async ({ params }: BasePageProps) => {
  const { locale } = await params;
  setRequestLocale(locale);
  const t = await getTranslations("IdPhotosPage");
  ...
  return (
    <>
      ...
      <section className="relative h-[50vh] min-h-[350px] overflow-hidden">
        <Image
          src="/images/services/id-photos.jpg"
          alt={t("title")}
          fill
          priority
